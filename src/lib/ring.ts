...
export class RingEnhancedSpookinatorV2 {
  ...
  constructor(configPath: string, debug: boolean = false) {
    ...
    this.ring.onRefreshTokenUpdated.subscribe(
      async ({ newRefreshToken, oldRefreshToken }) => {
        ...
        const fileContents = readFileSync(this.configPath, {
          encoding: "utf-8",
        });
  ...
  public addSensorCallback(
    sensor: RingDevice,
    callback: (data: RingDeviceData) => void,
  ): void {
  ...
  private async getAllContactSensors(): Promise<RingDevice[]> {
    ...
    for (const location of locations) {
      ...
      for (const device of devices) {
        log.info(
          `found device: ${device.name} - ${device.zid} (${device.deviceType})`,
        );
    ...
    return allDevices.filter(
      (device) => device.data.deviceType === RingDeviceType.ContactSensor,
    );
  }
}
