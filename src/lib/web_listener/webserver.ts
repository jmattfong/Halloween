...
export class OrchestratorWebServer {
  ...
  constructor(
    port: number,
    registerCallback: (clientUri: URL, sensors: string[]) => void,
    triggerCallback: (
      name: string,
      scope: TriggerScope,
      clientEndpoint?: string,
    ...
    let handler = async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method == "POST" && req.url == "/register") {
        ...
        try {
          registerEvent =
            await getJSONDataFromRequestStream<RegisterEvent>(req);
        } catch (e) {
        ...
        const clientUri = new URL(
          `http://${registerEvent.ip}:${registerEvent.port}`,
        );
  ...
  constructor(
    ...
    let handler = async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method == "POST" && req.url == "/event") {
        ...
        try {
          sensorUpdate =
            await getJSONDataFromRequestStream<SensorUpdateEvent>(req);
        } catch (e) {
          log.info(`error processing the input: ${e}`);
          res.statusCode = 400;
          res.end("error with request");
        ...
        log.debug(
          `received message: ${req.method} -> ${JSON.stringify(sensorUpdate)}`,
        );
        res.statusCode = 200;
        res.end("OK");
        return;
