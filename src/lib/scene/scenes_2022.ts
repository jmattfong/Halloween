    ...
    let events: Event[] = lights.map((light) => {
      return new Event(
        ...
        new RandomSoundPattern(
          ["resources/lightning_bolt.mp3", "resources/lightning_bolt_2.mp3"],
...
class FrontLightFlickerScene extends MultiPartScene {
  constructor(hueSensorId: number, lights: string[]) {
    let defaultLighting: Pattern = new OnPattern(RELAX, 1);
    let events: Event[] = lights.map((light) => {
      return new Event(light, new FlickerPattern(7), defaultLighting);
    });
    let unSpookyEvents: Event[] = lights.map((light) => {
      return new Event(light, defaultLighting);
    });
  ...
  constructor(mirrorLights: string[], showerLight: string) {
    ...
    spookyEvents.push(
      new Event(
        showerLight,
        new SoundPattern(
          "resources/David_2022/downstairs_bathroom.wav",
...
class WerewolfDoorJiggleScene extends Scene {
  ...
  constructor(hueSensor: number, dummyLight: string) {
    ...
    this.spookyEvent = new Event(
      dummyLight,
      new SoundPattern(
        "resources/David_2022/scratching_dog.wav",
        new SleepPattern(0),
        0,
      ),
    );
  ...
  constructor(lights: string[]) {
    let events: Event[] = lights.map((light) => {
      ...
      return new Event(
        light,
        new SoundPattern(
          "resources/David_2022/the_beast.wav",
  ...
  constructor(mirrorLights: string[], showerLight: string) {
    ...
    spookyEvents.push(
      new Event(
        showerLight,
        new SoundPattern(
          "resources/David_2022/guest_bathroom_psycho.wav",
  ...
  constructor(lights: string[]) {
    ...
    let hueEvents = lights.map((light) => {
      return new Event(
        light,
        new SoundPattern(
          "resources/David_2022/guest_bedroom_candy.wav",
          new OnPattern(RELAX, 10, 5),
          0,
          1,
          true,
    ...
    let ringEvents = lights.map((light) => {
      return new Event(
        light,
        new SoundPattern(
          "resources/David_2022/guest_bedroom.wav",
          new OffPattern(60, 1),
          0,
          1,
          true,
  ...
  getRepeatingEvents(...lightNames: string[]): Event[] {
    ...
    let events = [
      new Event(
        ...
        new SleepPattern(repeatTime),
        new RandomSoundPattern(
          [
            "resources/David_2022/rooftop_costume_contest.wav",
            "resources/David_2022/rooftop_feeding.wav",
            "resources/David_2022/rooftop_werewolf.wav",
          ],
          new OffPattern(60),
        ),
        new OnPattern(RELAX, 1),
...
export const SCENES_2022: { [key: string]: Scene } = {
  ...
  front_light_flicker: new FrontLightFlickerScene(2, [
    "living_room_1",
    "living_room_2",
  ]),
  ...
  creepy_clown_shower: new DownstairsBathCreepyClownShowerScene(
    ["down_bath_1", "down_bath_2"],
    "down_bath_3",
  ),
  halloween_hallway: new HalloweenHallway(
    "halloween_hallway_1",
    "halloween_hallway_2",
    "halloween_hallway_3",
    "halloween_hallway_4",
  ...
  guest_bathroom: new GuestBathroomScene(
    ["guest_bathroom_mirror_1", "guest_bathroom_mirror_2"],
    "guest_bathroom_shower",
  ),
