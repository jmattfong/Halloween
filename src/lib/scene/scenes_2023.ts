    ...
    let spookyEvents = lights.map((light) => {
      if (light == "16" || light == "19") {
        return new Event(
          light,
          new SoundPattern(
            `${RESOURCES_DIR}/costume_contest/costume_contest_23_gather.mp3`,
            new FlickerPattern(3),
            0,
          ),
          new OnPattern(RELAX, 20, 0.5),
    ...
    let spookyEvents = lights.map((light) => {
      if (light == "16" || light == "19") {
        return new Event(
          light,
          new SoundPattern(
            `${RESOURCES_DIR}/costume_contest/costume_contest_23_vote.mp3`,
            new FlickerPattern(3),
            0,
          ),
          new OnPattern(RED, 20, 0.5),
...
const LIGHTS = {
  living_room: ["16", "19"],
  front_walkway: ["24", "17"],
  downstairs_entry: ["8", "9"],
  downstairs_bathroom: ["1", "2", "3"],
  half_bathroom: ["6", "7"],
  guest_bathroom: ["20", "32", "22"],
  guest_bedroom: ["23", "31"],
  upstairs_hall: ["12", "13", "15"],
  switch: ["33"],
} as const;
