...
export class Chromecaster {
  ...
  constructor(
    ...
    baseServerUrl: string = "https://jmattfong-halloween.s3.us-west-2.amazonaws.com",
    debug: boolean = false,
  ) {
  ...
  private setupConnection(
    ...
    chromecast: any,
    client: any,
    onConnect: (error: Error, player: any) => void,
  ) {
    chromecast.on("serviceUp", function (service: any) {
      ...
      if (service.name !== deviceName) {
        log.info(
          `${service.name} does nawt match the requested device ${deviceName}`,
        );
        return;
      }
  ...
  public async start(): Promise<void> {
    ...
    log.info(
      "chromecast is ready. Starting spooky vids, with some spooky blank vids of course",
    );
    ...
    setInterval(() => {
      ...
      if (
        this.currentPlayingVideo.getVideoLengthSeconds() * 1000 - timePlayedMs <
        500
      ) {
        log.info(
          `Video ${this.currentPlayingVideo.getName()} over. Playing blank video`,
        );
        this.playBlankVideo();
      }
    }, 1000);
